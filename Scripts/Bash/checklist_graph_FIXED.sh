...
# FIXED: June 2026 - Improved TSV parsing to handle multi-column ARG output
###################################################################################################
...
while IFS= read -r graph_query; do
    ...
    if [[ -z "$check_guid" ]] || [[ $check_guid == $this_guid ]]; then
        ...
        if [[ "$graph_query" == "null" ]]; then
            ...
            # The query should return one line per result. Fields ID and Compliant are mandatory.
            # NOTE: ARG queries may return more than 2 columns. We explicitly request only id and compliant.
            query_result=$(az graph query -q "$graph_query" ${mg_option} -o tsv 2>$error_file --query 'data[].[id,compliant]' | sort -u)
            rm $error_file 2>/dev/null; touch $error_file
            # FIX: Properly parse tab-separated values (TSV) to handle multi-column output
            while IFS=$'\t' read -r resource_id compliant_value
            do
                if [[ -n "$resource_id" ]]; then
                    # Extract just the first character of the compliant value (should be 0, 1, true, false, etc.)
                    result_id=${compliant_value:0:1}
                    # Debug output to show what we're parsing
                    if [[ "$debug" == "yes" ]] && [[ "$result_id" != "0" ]] && [[ "$result_id" != "1" ]] && [[ "$result_id" != "t" ]] && [[ "$result_id" != "f" ]]; then 
                        echo "DEBUG: Parsed resource_id='${resource_id:0:80}...' compliant_value='${compliant_value}' result_id='$result_id'"
                    fi
                    # Print output in color if format is text
                    if [[ "$format" == "text" ]]; then
                        if [[ "$result_id" == "1" ]] || [[ "$result_id" == "t" ]]; then
                            result_text="compliant"
                            color=$green
                        elif [[ "$result_id" == "0" ]] || [[ "$result_id" == "f" ]]; then
                            result_text="non-compliant"
                            color=$red;
                        else
                            if [[ "$debug" == "yes" ]]; then echo "WARNING: unknown result code returned: '$result_id' (from compliant_value: '$compliant_value')"; fi
                    ...
                    if [[ "$debug" == "yes" ]]; then echo "DEBUG: Graph query returned an empty result"; fi
                fi
            done < <(printf '%s\n' "$query_result")
        fi
    fi
